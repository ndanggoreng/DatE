...
from log.activity_log import append_log, read_logs, clear_logs
from .paths import app_data_dir, res_dir
from .server import (
    ensure_data_files,
    HttpTransferServer,
    port_in_use,
...
class DataTransfer(QWidget):
    ...
    def __init__(self):
        ...
        self.url = ""
        self.i18n = I18n(res_dir(), app_data_dir())
    ...
    def _build_ui(self):
        ...
        self.copy_btn.setObjectName("copyLink")
        self.copy_btn.setFlat(True)
        self.copy_btn.setCursor(Qt.PointingHandCursor)
        self.copy_btn.clicked.connect(self.copy_address)
        url_layout.addWidget(self.copy_btn, 0, Qt.AlignVCenter)

        layout.addWidget(url_box)

        self.folder_btn = QPushButton()
        self.folder_btn.setObjectName("gridBtn")
        self.folder_btn.clicked.connect(self.select_folder)

        self.send_btn = QPushButton()
        self.send_btn.setObjectName("gridBtn")
        self.send_btn.clicked.connect(self.send_to_phone)

        action_grid = QGridLayout()
        action_grid.setSpacing(8)
        action_grid.setColumnStretch(0, 1)
        action_grid.setColumnStretch(1, 1)
        action_grid.addWidget(self.folder_btn, 0, 0)
        action_grid.addWidget(self.send_btn, 0, 1)
    ...
    def apply_language(self):
        ...
        self.send_btn.setText(t("send_to_phone"))
    ...
    def load_config(self):
        ensure_data_files(res_dir(), app_data_dir())
    ...
    def start_server(self):
        ...
        self._http_server = HttpTransferServer(res_dir(), app_data_dir(), port)
        ...
        append_log("server_start", f"{self.url}{port_hint}", "")
    ...
    def send_to_phone(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, self.i18n.t("send_to_phone_title")
        )
        if not path:
            return
        try:
            saved = queue_outgoing_file(path)
            QMessageBox.information(
                self,
                self.i18n.t("success"),
                f"{self.i18n.t('share_queued')}:\n{os.path.basename(saved)}",
            )
        except OSError as e:
            QMessageBox.warning(self, self.i18n.t("info"), str(e))
